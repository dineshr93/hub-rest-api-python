...
#
#
# Copyright (c) 2024 Dinesh Ravi. All rights reserved worldwide. 
#
...
function validate_inputs()
{
    ...
    if [[ -z "${ACCESS_TOKEN}"  || -z "${WEB_URL}" || -z "${PROJECT_NAME}" || -z "${VERSION_NAME}" || -z "${VERSION2_NAME}"|| -z "${OUTPUT_MAVEN_COORD_FILE}" ]]
    then
       echo "Script inputs missing please use the following format: ./export_bom.sh -u https://myblackduck -a <api_token> -p <project name> -v <version name> -w <version name> -f <output txt file>"
       exit 1
    # else
...
function dump_bom()
{
   ...
   # echo "==========================="
    ...
    local itemsAdd=$(echo "${bomAdded}" | jq -r '.items[] | .component.origins[].externalId')
    local itemsAddvs=$(echo "${bomAddedvs}" | jq -r '.items[] | .component.origins[].externalId')
    local itemsRemoved=$(echo "${bomRemoved}" | jq -r '.items[] | .component.origins[].externalId')
    local itemsRemovedvs=$(echo "${bomRemovedvs}" | jq -r '.items[] | .component.origins[].externalId')
    local itemsChanged=$(echo "${bomChanged}" | jq -r '.items[] | .component.origin[].externalId')
    local itemsChangedvs=$(echo "${bomChangedvs}" | jq -r '.items[] | .component.origins[].externalId')
   ...
   # echo "$itemsAddvs"    
   combined=$(echo "$itemsAdd"$'\n'"$itemsAddvs" | sort | uniq | grep -E '^[a-zA-Z0-9._-]+:[a-zA-Z0-9._-]+:[a-zA-Z0-9._-]+$')
   echo "$combined" > $OUTPUT_MAVEN_COORD_FILE


   echo "Total maven coordinates added $(cat $OUTPUT_MAVEN_COORD_FILE | wc -l) <-------------"
